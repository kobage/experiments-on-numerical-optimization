...
//>>>>>> Component 1: Change of Variables,  their representation and container of the tests <<<<< 
...
double sinusoidalSubstitution(double v) noexcept
{
	return 1 + sin(v);
}
double sinusoidalcSubstitutionPrime(double v) noexcept
{
	return cos(v);
}
double inverseSinusoidalSubstitution(double v) noexcept
{
	return asin(v - 1);
}
...
void runTests_LB(int m = mVALUE)
{
	...
	//-----------Setting tolerance and size-----------------//
	n = lb_testsVector[i].size;							//setting tsize
	if (lb_testsVector[i].fNameComposite == sgComposite || lb_testsVector[i].fNameComposite == sgQuadraticComposite)
		u_min::setDimentions(lb_testsVector[i].size, 1E-6, 100);			//setting tolerance 
	else
		u_min::setDimentions(lb_testsVector[i].size, 1E-6, m);			//setting tolerance 
	...
	for (int j = 0; j<repNumber; j++)
	{
		...
		if (lb_testsVector[i].fNameComposite == sgComposite || lb_testsVector[i].fNameComposite == sgQuadraticComposite)
			u_min::l_bfgs(lnSrch, lb_testsVector[i].fNameComposite, stoppingByDeviation,100);
		else
			u_min::l_bfgs(lnSrch, lb_testsVector[i].fNameComposite, stoppingByProjGradInfNorm_LB,m);
